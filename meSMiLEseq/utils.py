...
def calculate_stats2(dGG):
    """Calculate the letter sizes of mg and CF form dGG logos, but take only values into account if they are > 0
...
def pfm_to_ppm(df):
    ...
    return df.apply(lambda x: _col_wise_propability(x))
...
def make_logo(logomatrix, ax=None, title=None):
    """
    Plots a logo on the given Axes.
